...
public class ImageTarget : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        if (toDraw == null)
        {
            toDraw = Resources.GetBuiltinResource<Mesh>("Plane.fbx");
        }
        ...
        Gizmos.DrawMesh(toDraw, position, rotation, new Vector3(imageWidth, 1, imageHeight)); // divide by 10 because the default size of a plane in Unity is 10mx10m when the scale is 1,1,1. So if we want a size matching the physical world at scale 1,1,1, we have to divide the visualization scale by 10.
        //Gizmos.Draw
    }
}
